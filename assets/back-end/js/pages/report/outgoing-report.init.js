...
function action_method() {
  ...
  if (ef.checkValidity() === false) {
    ...
    ef.classList.remove("was-validated");
    $.ajax({
        url: baseURL+'outgoing_report/report_outmail',
        data: formData,
        type: "POST",
        contentType: false,
        processData:false,
        ...
        success: function(data){
          ...
          for(i=0; i<data.length; i++){
              const dtsurat = new Date(data[i].OutMailDate);
              ...
              const dtterima = new Date(data[i].OutMailLog);
              ...
              html += '<tr>'+
                          ...
                          '<td><p class="text-truncate mb-0"><b>'+data[i].OutMailDestination+'</b></p><p class="text-muted mb-0">'+data[i].OutMailNumber+'</p></td>'+
