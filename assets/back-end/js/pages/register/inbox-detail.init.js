...
function method_resetcomment() {
  ef = document.getElementById("form-commentar");
  ef.classList.remove("was-validated");
...
function method_actionattachment(id) {
  ...
  url = baseURL+'mail_detail/add_inmailattachment/'+id;
  ...
  if (ef.checkValidity() === false) {
      ...
      $.ajax({
          ...
          success: function(data){
            ...
            var notifier = new Notifier();
            var notification = notifier.notify("success", "<b>201 Request Successfully Created</b> <br>Berhasil mengunggah lampiran pada surat masuk tersebut");
            notification.push();
            return;
          },
          error: function (jqXHR, textStatus, errorThrown) {
...
function check_duplicatename(id) {
  ...
  if (method_type == 'add_attachment' && name) {
    $.ajax({
        url: baseURL+"mail_detail/check_duplicatename/"+id,
        type: "GET",
        data:"name=" + name,
        contentType: false,
        processData:false,
      ...
      success: function(data){
        ...
        if (data.length == 0) {
          ...
          for(i=0; i<data.length; i++){
              ...
              html += '<tr>'+
                          '<td>'+
                            '<button type="button" class="close" aria-label="Close" onclick="delete_inmailattachment('+data[i].AttachmentID+')">'+
...
function delete_inmailattachment(idatt) {
  Swal.fire({
      ...
      if(result.value) {
          $.ajax({
              url: baseURL+'mail_detail/delete_inmailattachment/'+idatt,
              type: "POST",
              cache: false,
              async: true,
              dataType: "JSON",
