! function(t) {
  ...
  t("#side-menu").metisMenu(), t("#vertical-menu-btn").on("click", function(e) {
      ...
      t('[data-provide="datepicker"]').datepicker({
        autoclose: true,
      })
    }),
    t(function() {
      t('[data-provide="maxlength"]').maxlength({
        alwaysShow: !0,
    ...
    function() {
      ...
      t("#status").fadeOut(), t("#preloader").delay(350).fadeOut("slow"), t(".btn-logout").on("click", function () { destroy_auth() });
    }), Waves.init()
}(jQuery);
...
function destroy_auth() {
  Swal.fire({
      ...
      if(result.value) {
          $.ajax({
              url: baseURL+'auth/destroy_login',
              type: "GET",
              cache: false,
              async: true,
              dataType: "JSON",
